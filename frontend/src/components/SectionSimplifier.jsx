    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          {sections.map((sec, idx) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 rounded-xl border transition-all relative interactive-card animate-fade-slide focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none ${
                  isSelected
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-mono font-bold text-blue-700 bg-blue-100/70 border border-blue-200/60 px-2 py-0.5 rounded shadow-2xs">
                      {sec.id}
                    </span>
                    <h4 className="text-sm font-bold text-slate-900 tracking-tight">{sec.title}</h4>
                  </div>
                  <span className={`text-[10px] font-semibold uppercase px-2.5 py-0.5 rounded-full border shadow-2xs ${getCategoryBadgeClass(sec.category)}`}>
                    {sec.category}
                  </span>
                </div>

                <div className="text-xs text-slate-800 leading-relaxed font-medium mt-2 bg-slate-50/60 p-3.5 rounded-xl border border-slate-100">
                  {sec.plain_english}
                </div>
                ...
                {sec.key_takeaways && sec.key_takeaways.length > 0 && (
                  <div className="mt-3 space-y-1.5">
                    ...
                    <ul className="space-y-1 text-xs text-slate-600">
                      {sec.key_takeaways.map((takeaway, tIdx) => (
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] text-blue-600 font-semibold">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>Trace to original clause</span>
                  </span>
        ...
        <div className="lg:col-span-5 sticky top-24">
          <div className="bg-slate-950 text-slate-200 rounded-2xl shadow-xl border border-slate-800 overflow-hidden flex flex-col h-[650px]">
            <div className="px-4 py-3 bg-slate-900 border-b border-slate-800 flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono bg-blue-950 text-blue-300 px-2.5 py-0.5 rounded border border-blue-800/80 font-bold">
            ...
            <div
              ref={originalPaneRef}
              className="p-4 overflow-y-auto space-y-4 text-xs font-mono leading-relaxed text-slate-300 flex-1 scroll-smooth"
            >
              {sections.map((sec) => {
                const isSelected = activeSectionId === sec.id;
                return (
                  <div
                    ...
                    className={`p-3.5 rounded-xl transition-all duration-300 cursor-pointer border ${
                      isSelected
                        ? 'bg-blue-950/80 border-blue-500 ring-2 ring-blue-500/40 text-white trace-active-flash shadow-lg'
                        : 'bg-slate-900/50 border-slate-800/80 hover:border-slate-700 text-slate-400'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1.5 pb-1 border-b border-slate-800 text-[11px] text-blue-400 font-semibold">
                      <span>[{sec.id}] {sec.title}</span>
                      {isSelected && (
                        <span className="text-[10px] bg-blue-600 text-white px-2 py-0.5 rounded font-bold uppercase tracking-wider shadow-xs">
                          Active Trace
                        </span>
                      )}
                    </div>
